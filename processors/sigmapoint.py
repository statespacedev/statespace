...
class SigmaPoint():
    ...
    def spfcholesky(self, model):
        ...
        sim, f, h, F, H, R, Q, G, x, P = model.ekf()
        f, h, Xtil, Ytil, X1, Pxy, W, S, Sproc, Sobs = model.spcho()
        for t, o, u in sim():
            x, S, X = self.temporal(x, f, Xtil, X1, W, S, Sproc, u)
            x, S, y = self.observational(x, o, h, X, Xtil, Ytil, Pxy, W, S, Sobs)
            self.log.append([t, x, y])
    ...
    def temporal(self, x, f, Xtil, X1, W, S, Sproc, u):
    ...
    def observational(self, x, o, h, X, Xtil, Ytil, Pxy, W, S, Sobs):
