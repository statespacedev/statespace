...
def yhat(xref, xhat):
    return y(xref, 0) + (2 * xref + 3 * xref**2) * (xhat - xref)
...
yhatts[0] = yhat(xrefts[0], xhatts[0])
...
for tk in range(1, n):
    ...
    yts[tk] = y(xrefts[tk], vts[tk])
    xhatts[tk] = xhattr(xrefts[tk-1], xhatts[tk-1])
    Ptilts[tk] = Ptiltr(xrefts[tk-1], Ptilts[tk-1])
    yhatts[tk] = yhat(xrefts[tk], xhatts[tk])
    ets[tk] =  yts[tk] - yhatts[tk]
    Reets[tk] = Ree(xhatts[tk], Ptilts[tk])
    Kts[tk] = K(Ptilts[tk], xrefts[tk], Reets[tk])
    xhatts[tk] = xhatup(xhatts[tk], Kts[tk], ets[tk])
    Ptilts[tk] = Ptilup(Kts[tk], xrefts[tk], Ptilts[tk])
    xtilts[tk] = xhatts[tk] - xts[tk]

plots.xts(ets, tts, end=40)
