...
around our model and external reality - we're at the least in the regions of the 'genetic algorithms' -
survival-of-the-fittest is a core concept here, but the 'genetic material' concept is de-emphasized in favor of using
...
the particles live and die, and new ones are born via survival-of-the-fittest 'resampling'. in fact,
new ones essentially kill old ones - things are quite red of tooth and claw. basic idea is - when new data comes in,
...
x components, spkf gives us 2x + 1 particles - two sigma point particles for each state component, and one particle
representing the estimated mean state. with sequential monte carlo, we now simply have a 'cloud' of particles
swarming about over time - at any time we can measure the cloud's estimated mean state using kernel density methods,
