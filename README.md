...
210328 new model structure is definitely on the right path - it's already quite essential for working on the new bearings only model. evaluation and plotting is migarting into a new eval class within the base model. there each model will know how to make it's own appropriate plots - definitely including model states and processor states - and perform it's own evaluation tests. 
